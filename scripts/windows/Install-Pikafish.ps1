...
if ((Test-Path $executable) -and (Test-Path $network)) {
...
try {
  ...
  if (-not (Test-Path $stagedExe) -or -not (Test-Path $stagedNetwork)) {
    throw 'The official Pikafish archive did not contain the expected AVX2 engine and NNUE network.'
  }
