...
object Duel:
  ...
  type UsernameRank = (user: UserName, rank: RankSnapshot)

  case class DuelPlayer(name: UserName, rank: RankSnapshot, tournamentRank: Rank)
  object DuelPlayer:
    given UserIdOf[DuelPlayer] = _.name.id

  def tbUser(p: UsernameRank, ranking: Ranking) =
    ranking.get(p.user.id).map { rank =>
      DuelPlayer(p.user, p.rank, rank + 1)
    }

  private[tournament] val rankOrdering = Ordering.by[Duel, Int](_.averageRankOrdinal)
...
final private class DuelStore:
  ...
  def bestRanked(tourId: TourId, nb: Int): List[Duel] =
    get(tourId).so:
      scalalib.HeapSort.topNToList(_, nb)(using rankOrdering)

  def find(tour: Tournament, user: UserId): Option[GameId] =
    get(tour.id).flatMap { _.find(_.has(user)).map(_.gameId) }

  def add(tour: TourId, game: GameId, p1: UsernameRank, p2: UsernameRank, ranking: Ranking): Unit =
  ...
  def makeDuel(game: GameId, p1: UsernameRank, p2: UsernameRank, ranking: Ranking): Option[Duel] = for
  ...
  yield Duel(
    ...
    p1 = p1,
    p2 = p2,
    averageRankOrdinal = (p1.rank.ordinal + p2.rank.ordinal) / 2
  )
  ...
  def remove(game: GameId, tour: TourId): Unit =
    byTourId.computeIfPresent(tour): tb =>
      ...
      val w = tb - Duel(game, null, null, 0)
